...
void Timer_ResetAll() { memset(timers, 0, sizeof(timers)); }
        ...
        for (gl_tmr_idx = 0u; gl_tmr_idx < MAX_TIMER_NUMB; gl_tmr_idx++) {
            ...
            if (gl_curr_tmr->is_active != false) {
                ...
                if (gl_curr_tmr->curr_time <= 0u) {
                    ...
                    if (gl_curr_tmr->curr_time == 0u) memset(gl_curr_tmr, 0u, sizeof(timers_t));
                }
            }
        }
...
void Timer_TickCbk(void) { tmr_tick_ready = true; }
...
bool Util_IsFlagChecked(uint32_t* event_map, uint8_t event_sel) {
    ...
    if ((evt_stat & (uint32_t)(1UL << event_sel)) != 0U) ret = true;
