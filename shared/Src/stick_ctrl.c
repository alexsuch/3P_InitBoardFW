...
static void Stick_Process(void)
{
	...
	if (
			(pwm_1_state == STICK_STATE_NONE) ||
			(pwm_2_state == STICK_STATE_NONE)
		)
	{
		return;
	}
	...
	else if (pwm_1_state == STICK_STATE_ACTIVE_POS_1)
	{
		new_ctrl_evt = CONTROL_EVT_DISARM;
				...
				(pwm_1_state == STICK_STATE_ACTIVE_POS_2) &&
				(pwm_2_state >= STICK_STATE_ACTIVE_POS_1)
			)
	{
		new_ctrl_evt = CONTROL_EVT_ARM_WO_ACC;
	}
	else if (
				((pwm_1_state == STICK_STATE_ACTIVE_POS_3)  &&
				 (pwm_2_state >= STICK_STATE_ACTIVE_POS_1)) ||
				 (pwm_1_state == STICK_STATE_ACTIVE_POS_2)
			)
	{
