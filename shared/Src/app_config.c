...
static bool AppGonfig_ReadFlashConfig(uint32_t addr, config_data_t* cfg_data) {
    ...
    if (IS_FLASH_MAIN_MEM_ADDRESS(addr) != 0u) {
        ...
        if ((AppGonfig_IsAllElements((uint8_t*)addr, sizeof(config_save_data_t), 0x00)) ||
...
bool AppConfig_SaveConfiguration(config_data_t* config) {
    ...
    // https://community.st.com/t5/stm32-mcus-products/quot-warning-lt-elffile-gt-has-a-load-segment-with-rwx/td-p/83148  (Adding "-Wl,--no-warn-rwx-segment"
