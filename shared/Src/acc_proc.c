...
int64_t vectorMagnitudeScaled(int16_t x, int16_t y, int16_t z) { return (int64_t)x * x + (int64_t)y * y + (int64_t)z * z; }

int64_t dotProduct(int16_t x1, int16_t y1, int16_t z1, int16_t x2, int16_t y2, int16_t z2) { return (int64_t)x1 * x2 + (int64_t)y1 * y2 + (int64_t)z1 * z2; }
...
int64_t absInt64(int64_t v) { return (v < 0) ? -v : v; }
...
static void App_AccCbk(system_evt_t evt, uint32_t usr_data) {
    if (evt == SYSTEM_EVT_ERROR) {
        ...
        if (usr_data == ACC_EVT_INIT_OK) {
            ...
            if ((accProcStatus.hit_detection_enabled != false) || (accProcStatus.move_detection_enabled != false)
#if ACC_SHAKE_DETECTION_ENABLE
    ...
    if (accProcStatus.data_ready != false) {
        if (accProcStatus.init_skip_counts == 0) {
            if ((accProcStatus.hit_detection_enabled != false)
                ...
                if (accProcStatus.validCount < ACC_BUFF_SIZE) accProcStatus.validCount++;
                ...
                if (accProcStatus.validCount >= ACC_BUFF_SIZE) {
                    ...
                    tmp32 = accProcStatus.x_sum * accProcStatus.x_sum + accProcStatus.y_sum * accProcStatus.y_sum + accProcStatus.z_sum * accProcStatus.z_sum;
#else
                ...
                accProcStatus.net_x_sum_buff[accProcStatus.net_idx] = accProcStatus.net_x_sum += *x_axis - accProcStatus.net_x_raw_buff[oldest_idx];
                accProcStatus.net_y_sum_buff[accProcStatus.net_idx] = accProcStatus.net_y_sum += *y_axis - accProcStatus.net_y_raw_buff[oldest_idx];
                accProcStatus.net_z_sum_buff[accProcStatus.net_idx] = accProcStatus.net_z_sum += *z_axis - accProcStatus.net_z_raw_buff[oldest_idx];
