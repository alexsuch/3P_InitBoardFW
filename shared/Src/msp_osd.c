...
const char* const OSD_MESSAGE_LUT[] = {[OSD_MSG_BOOM] = "BOOM!!!",
 ...
 */
static bool format_timer_string(char* timer_str, uint16_t timer_seconds, osd_message_index_t prefix_index, osd_message_index_t minutes_suffix_index,
 ...
 * @return true if formatting successful, false on error
 */
static bool format_osd_message_from_system_info(
    const init_board_system_info_t* system_info, char* output_buffer, uint16_t buffer_size, bool error_blink_state) {
    ...
    bool has_active_timer = (system_info->board_state != BOARD_STATE_CHARGING && system_info->timer_mode != TIMER_MODE_NONE && system_info->timer_seconds > 0);
    ...
    if (has_active_timer) {
        ...
        switch (system_info->timer_mode) {
            case TIMER_MODE_SAFE:
                ...
                timer_formatted = format_timer_string(
                    timer_str, system_info->timer_seconds, OSD_MSG_TIMER_SAFE_PREFIX, OSD_MSG_TIMER_DESTROY_SUFFIX, OSD_MSG_TIMER_SAFE_SUFFIX);
            ...
            case TIMER_MODE_SELF_DESTROY:
                ...
                timer_formatted = format_timer_string(
                    timer_str, system_info->timer_seconds, OSD_MSG_TIMER_DESTROY_PREFIX, OSD_MSG_TIMER_DESTROY_SUFFIX, OSD_MSG_TIMER_SAFE_SUFFIX);
                break;

            case TIMER_MODE_MINING:
                // Mining timer - таймер активації мінного режиму, показуємо з префіксом "M:"
                timer_formatted = format_timer_string(
                    timer_str, system_info->timer_seconds, OSD_MSG_TIMER_MINING_PREFIX, OSD_MSG_TIMER_DESTROY_SUFFIX, OSD_MSG_TIMER_MINING_SUFFIX);
                break;
