...
void Indication_Reset(uint8_t* system_error_code_ptr) {
    ...
    indStatus.system_error_code_ptr = system_error_code_ptr;
    Indication_ClearStatusIndication();
    Indication_ClearErrorIndication();
}
...
static void Indication_UpdateErr(err_code_t err_code) {
    /* Update system error code if pointer is valid */
    if (indStatus.system_error_code_ptr != NULL) {
        *indStatus.system_error_code_ptr = (uint8_t)err_code;
    }
    ...
    switch (err_code) {
        ...
        case ERR_CODE_UNEXPECTED_VUSA_SHORTED:
            ...
            break;
        case ERR_CODE_ACCELEROMETER_FAIL:
            /* Set periodic error */
...
void Indication_SetError(err_code_t err_code) {
    ...
    if ((indStatus.err_code_mask == 0) || ((indStatus.err_code_mask != 0) && (Indication_GetHighErrIdx(indStatus.err_code_mask) > err_code))) {
        /* Set error code mask */
...
void Indication_ClearError(err_code_t err_code) {
    ...
    if ((indStatus.err_code_mask != 0) && (Indication_GetHighErrIdx(indStatus.err_code_mask) >= err_code)) {
        ...
        if (indStatus.err_code_mask != 0) {
            ...
            /* No more errors - clear system error code */
            if (indStatus.system_error_code_ptr != NULL) {
                *indStatus.system_error_code_ptr = 0;
            }
        }
    } else {
...
void Indication_SetStatus(ind_status_t status_code, uint32_t user_data) {
    ...
    if ((indStatus.err_code_mask == 0) || (status_code == IND_STATUS_CONFIGURATION_MODE)) {
        if (((indStatus.curr_status == status_code) && (status_code == IND_STATUS_FUSE_POPULATED)) ||
            ((indStatus.curr_status == IND_STATUS_CONFIGURATION_MODE) && (status_code != IND_STATUS_CONFIGURATION_APPLIED))) {
