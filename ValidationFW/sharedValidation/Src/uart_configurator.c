...
static bool status_send_flag, resp_send_flag;
...
static bool parse_status = false;
			...
			if (byte == START_BYTE)
			{
				...
				else
				{
					...
					if (data_len > 0)
					{
						...
						parse_status = App_ParseTestCommand(uart_config.decoded_rd_pkt, data_len);
...
bool UartConfig_SendOneByteData (uint8_t cmd, uint8_t status, uint8_t data)
{
	memset(uart_config.wr_pkt, 0u, sizeof(uart_config.wr_pkt));

	uart_config.wr_pkt[TEST_CMD_IDX] = cmd;
	uart_config.wr_pkt[TEST_STATUS_IDX] = status;
	uart_config.wr_pkt[TEST_DATA_START_IDX] = data;
...
bool UartConfig_SendTwoByteData (uint8_t cmd, uint8_t status, uint8_t data1, uint8_t data2)
{
	memset(uart_config.wr_pkt, 0u, sizeof(uart_config.wr_pkt));

	uart_config.wr_pkt[TEST_CMD_IDX] = cmd;
	uart_config.wr_pkt[TEST_STATUS_IDX] = status;
	uart_config.wr_pkt[TEST_DATA_START_IDX] = data1;
	uart_config.wr_pkt[TEST_DATA_START_IDX + 1] = data2;
...
bool UartConfig_SendFourByteData (uint8_t cmd, uint8_t status, uint8_t data1, uint8_t data2, uint8_t data3, uint8_t data4)
{
	memset(uart_config.wr_pkt, 0u, sizeof(uart_config.wr_pkt));
	...
	uart_config.wr_pkt[TEST_DATA_START_IDX] = data1;
	uart_config.wr_pkt[TEST_DATA_START_IDX + 1] = data2;
	uart_config.wr_pkt[TEST_DATA_START_IDX + 2] = data3;
	uart_config.wr_pkt[TEST_DATA_START_IDX + 3] = data4;
...
bool UartConfig_Task(void)
{
	...
	{
		/* Prepare send packet */
		len = UartConfig_EncodePkt(uart_config.wr_pkt, uart_config.temp_wr_buff, TEST_PKT_DATA_SIZE);
