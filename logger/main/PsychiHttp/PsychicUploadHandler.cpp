...
bool PsychicUploadHandler::canHandle(PsychicRequest *request) { return true; }

esp_err_t PsychicUploadHandler::handleRequest(PsychicRequest *request) {
    esp_err_t err = ESP_OK;

    ESP_LOGI(PH_TAG, "PsychicUploadHandler::handleRequest called");
    ESP_LOGI(PH_TAG, "Content-Length: %d, Content-Type: %s", request->contentLength(), request->header("Content-Type").c_str());
    ...
    ESP_LOGI(PH_TAG, "Skipping parent handleRequest to preserve request body for multipart parsing");
    ...
    // Load parameters from query string only (not from multipart body)
    ESP_LOGI(PH_TAG, "Loading query parameters only");
    request->loadParams();
    ...
    ESP_LOGI(PH_TAG, "Checking if multipart: %s", request->isMultipart() ? "true" : "false");
    if (request->isMultipart()) {
        ESP_LOGI(PH_TAG, "Using multipart upload handler");
        err = _multipartUploadHandler(request);
    } else {
        ESP_LOGI(PH_TAG, "Using basic upload handler");
        err = _basicUploadHandler(request);
    }

    ESP_LOGI(PH_TAG, "Upload handler completed with result: %s", esp_err_to_name(err));

    // we can also call onRequest for some final processing and response
    if (err == ESP_OK) {
        ESP_LOGI(PH_TAG, "Calling onRequest callback");
    ...
    ESP_LOGI(PH_TAG, "handleRequest returning: %s", esp_err_to_name(err));
    return err;
}
...
esp_err_t PsychicUploadHandler::_multipartUploadHandler(PsychicRequest *request) {
    ...
    ESP_LOGI(PH_TAG, "Starting multipart upload handler");
    String value = request->header("Content-Type");
    ESP_LOGI(PH_TAG, "Content-Type header: %s", value.c_str());

    if (value.startsWith("multipart/")) {
        _boundary = value.substring(value.indexOf('=') + 1);
        _boundary.replace("\"", "");
        ESP_LOGI(PH_TAG, "Extracted boundary: %s", _boundary.c_str());
    } else {
    ...
    /* Content length of the request gives the size of the file being uploaded */
    int remaining = request->contentLength();
    ESP_LOGI(PH_TAG, "Starting multipart parsing - content length: %d", remaining);
    ESP_LOGI(PH_TAG, "Available heap before multipart parsing: %d bytes", esp_get_free_heap_size());

    char *buf = (char *)malloc(FILE_CHUNK_SIZE);
    if (!buf) {
        ESP_LOGE(PH_TAG, "Failed to allocate buffer for multipart upload");
        ESP_LOGE(PH_TAG, "Available heap: %d bytes, requested: %d bytes", esp_get_free_heap_size(), FILE_CHUNK_SIZE);
        return ESP_FAIL;
    }
    ESP_LOGI(PH_TAG, "Allocated %d byte buffer successfully", FILE_CHUNK_SIZE);
    ...
    while (remaining > 0) {
        ...
        ESP_LOGD(PH_TAG, "Remaining size: %d, attempting to receive up to %d bytes", remaining, std::min(remaining, FILE_CHUNK_SIZE));

        /* Receive the file part by part into a buffer with timeout */
        ESP_LOGD(PH_TAG, "Calling httpd_req_recv with timeout...");
        received = httpd_req_recv(request->request(), buf, std::min(remaining, FILE_CHUNK_SIZE));
        ESP_LOGD(PH_TAG, "httpd_req_recv returned: %d", received);

        if (received <= 0) {
            /* Retry if timeout occurred */
            if (received == HTTPD_SOCK_ERR_TIMEOUT) {
                ESP_LOGW(PH_TAG, "Socket timeout, retrying...");
                continue;
            }
            // bail if we got an error
            else if (received == HTTPD_SOCK_ERR_FAIL) {
                ESP_LOGE(PH_TAG, "Socket error: %d", received);
                err = ESP_FAIL;
                break;
            }
        }
        ...
        for (int i = 0; i < received; i++) {
            ...
            // Check heap every 100 bytes to detect memory issues
            if (index % 100 == 0) {
                size_t current_heap = esp_get_free_heap_size();
                if (current_heap < 10000) {
                    ESP_LOGW(PH_TAG, "Low heap during parsing: %d bytes at index %lu", current_heap, index);
                }
            }
...
void PsychicUploadHandler::_handleUploadByte(uint8_t data, bool last) {
    ...
    if (last || _itemBufferIndex == FILE_CHUNK_SIZE) {
        if (_uploadCallback) _uploadCallback(_request, _itemFilename, _itemSize - _itemBufferIndex, _itemBuffer, _itemBufferIndex, last);
        _itemBufferIndex = 0;
    }
}
    ...
    if (_multiParseState == WAIT_FOR_RETURN1) {
        ...
        if (_boundaryPosition < _boundary.length() && _boundary.c_str()[_boundaryPosition] != data) {
            ...
            if (!_itemIsFile) {
                ...
                if (_itemSize) {
                    if (_uploadCallback) _uploadCallback(_request, _itemFilename, _itemSize - _itemBufferIndex, _itemBuffer, _itemBufferIndex, true);
                    _itemBufferIndex = 0;
