...
PsychicEndpoint::PsychicEndpoint() : _server(NULL), _uri(""), _method(HTTP_GET), _handler(NULL) {}
...
PsychicEndpoint *PsychicEndpoint::setHandler(PsychicHandler *handler) {
    ...
    if (_handler != NULL) delete _handler;
...
PsychicHandler *PsychicEndpoint::handler() { return _handler; }

String PsychicEndpoint::uri() { return _uri; }
...
esp_err_t PsychicEndpoint::requestCallback(httpd_req_t *req) {
    ...
    if (handler != NULL) {
        if (handler->filter(&request) && handler->canHandle(&request)) {
            ...
            if (handler->needsAuthentication(&request)) return handler->authenticate(&request);
...
PsychicEndpoint *PsychicEndpoint::setAuthentication(const char *username, const char *password, HTTPAuthMethod method, const char *realm,
