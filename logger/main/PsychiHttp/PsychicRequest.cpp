...
{
  if (decode)
    return addParam(new PsychicWebParameter(PsychicHttp::urlDecode(name.c_str()), PsychicHttp::urlDecode(value.c_str()), post));
  else
...
const String PsychicRequest::_extractParam(const String& authReq, const String& param, const char delimit)
{
  ...
  if (_begin == String::npos)
    return "";
  size_t start = _begin + param.length();
  size_t end = authReq.find(delimit, start);
  if (end == String::npos)
    return "";
  return authReq.substr(start, end - start);
}
